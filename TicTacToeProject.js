const ticTacToeGame = {
     //grabing  all the elements with the class:"square" will return a HTMLCollection,  
    //then we convert this HTMLCollection to an array using the function Array.from
 ...
 //start the game.
  startGame()  
  {
    //initialize the game.
      ...
      {
         //add a eventListner for every element in the squareArray. 
...
//create a colorful box with the sign inside it ,for every player move.
createColorfulBox(squareNumber,boxColor){
  if(this.squareArray[squareNumber].textContent === ''){
      ...
      if(boxColor === 'blue')
      {
          ...
          this.squareArray[squareNumber].appendChild(colorfulBox); 
 }
},
  //push the current symbol to the symbol array.       
  pushSymbolToArraySymbol(element,index)
  {
      let currentSymbol = 'O' ; 
